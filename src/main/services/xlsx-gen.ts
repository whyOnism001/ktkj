...
export class XlsxGenerator {
  async generate(quote: Quote, outputPath: string): Promise<void> {
    ...
    for (const item of quote.items) {
      ...
      const specText = item.configDesc
        ? `${item.specs}\n${item.configDesc}`
        : item.specs
    ...
    const addSummaryRow = (
      ...
      valueCell.font = { bold, size: 12, name: '微软雅黑', color: { argb: bold ? `FF${ORANGE}` : `FF${BLUE_DARK}` } }
